...
Bitboard pieceMovement[pieceTypes][64];
...
void nonsliding_pieces() {
  for (uint8_t square = 0; square < 64; square++) {
    // pawn movement
    pieceMovement[W_PAWN][square] = 0;
    pieceMovement[B_PAWN][square] = 0;

    // no moves when reaching promotion rank
    if (square / 8 < 7)
      pieceMovement[W_PAWN][square] |= 1ULL << (square + 8);
    if (square / 8 == 1)
      pieceMovement[W_PAWN][square] |= (1ULL << (square + 16));
    
    if (square / 8 > 0)
      pieceMovement[B_PAWN][square] |= 1ULL << (square - 8);
    if (square / 8 == 6)
      pieceMovement[B_PAWN][square] |= (1ULL << (square - 16));

    // knight movement
    pieceMovement[W_KNIGHT][square] = 0;
    ...
    for (int8_t dir : knightDirs) {
      pieceMovement[W_KNIGHT][square] |= knightBox & (1ULL << (square + dir));
    }

    pieceMovement[W_KING][square] = 0;
    for (uint8_t y = max(square / 8 - 1, 0); y <= min(square / 8 + 1, 7); y++) {
      for (uint8_t x = max(square % 8 - 1, 0); x <= min(square % 8 + 1, 7); x++) {
        pieceMovement[W_KING][square] |= 1ULL << (y * 8 + x);
      }
    }
    pieceMovement[B_KNIGHT][square] = pieceMovement[W_KNIGHT][square];
    pieceMovement[B_KING][square] = pieceMovement[W_KING][square];
  }
}